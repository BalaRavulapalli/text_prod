...
def mcq(qg, results, payload,  selected_specific, coref_sents, executor, uniqueUserId):
    MCQ_URL = "https://dockermcqv4gcp-sn6tlr3gzq-uc.a.run.app/getquestion"
...
class TF:
    ...
    def tf(self, selected_specific, coref_sents, gpt2_completions, executor):
        ...
        for selected_item in selected_specific:
            ...
            if split_sentence:
                def query(text):
                    data = json.dumps({"inputs": text, "parameters": {"top_k": 30, "repitition_penalty": 10.0, "top_p": .80,
                                      "num_return_sequences": 10,  "max_length": int(len(text)*.7)}, "options": {"wait_for_model": True, "use_cache": False}})
                    # print('before')
...
#             generated_sentences.append(nltk.sent_tokenize(
#                 sample_output['generated_text'])[0])
...
class QuestionsForm(FlaskForm):
    ...
    def validate_url(form, field):
        ...
        if ((len(form.content.data) <= 10) or (len(form.content.data) >= 5000)) and (form.url.data == ""):
            raise ValidationError(
                "Input must be longer than 10 characters and shorter than 5000 characters")
...
def callback():
    ...
    # print(request.url)
    # token_url, headers, body = client.prepare_token_request(token_endpoint, authorization_response=(request.url.replace("http://localhost", "https://balapoc.southcentralus.cloudapp.azure.com")),
    #                                                         redirect_url="https://balapoc.southcentralus.cloudapp.azure.com:5000/login/callback", code=code)
...
def coldstart():
    with Executor() as executor:
        ...
        def cold_start(url):
            ...
            # tester[url] = True
            return json.loads(response.content.decode("utf-8"))
        for url in ['https://dockerfbgcp-sn6tlr3gzq-uc.a.run.app', 'https://dockermcqv4gcp-sn6tlr3gzq-uc.a.run.app', 'https://dockerprepv2gcp-sn6tlr3gzq-uc.a.run.app/', 'https://dockertfv2gcp-sn6tlr3gzq-uc.a.run.app']:
...
def new():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            url_mapping = {'fb': 'https://dockerfbgcp-sn6tlr3gzq-uc.a.run.app', 'mcq': 'https://dockermcqv4gcp-sn6tlr3gzq-uc.a.run.app',
            ...
            with Executor() as executor:
                ...
                if len(payload['input_text']) > 10:
                    ...
                    filter_coref_response = requests.request("POST", 'https://dockerprepv2gcp-sn6tlr3gzq-uc.a.run.app/filter_coref', data=json.dumps({
                    ...
                    if 'Multiple Choice' in payload['question_types']:
                        ...
                        if selected_specific[start:start+payload['max_questions']['Multiple Choice']]:
                            ...
                            mcq_request = mcq(
                                myqg, results, payload, selected_specific[start:start+payload['max_questions']['Multiple Choice']], coref_sents, executor, uniqueUserId)
                    ...
                    if 'Fill in the Blanks' in payload['question_types']:
                        ...
                        if selected_specific[start:start+payload['max_questions']['Fill in the Blanks']]:
                            ...
                            fb_request = fitb(
                                results, payload, selected_specific[start:start+payload['max_questions']['Fill in the Blanks']], coref_sents, executor)
                        # offset += payload['max_questions']['Fill in the Blanks']
                        logging.error(
                    ...
                    if 'Fill in the Blanks' in payload['question_types']:
                        ...
                        if selected_specific[start:start+payload['max_questions']['Fill in the Blanks']]:
                            ...
                            # print(sampled_dict)
                            # try:
                            logging.error('output sents fb ' +
                                          str(len(fb_output[0]['questions'])))
...
def Kahootxlsx():
    ...
    #     all_fb_ans.append(group[1]["answers"])
    # for group in data['Fill in the Blanks']:
    #     temp = ['', '', '', '', '', '60', '']
    #     temp[0] = group[0]['questions']
    #     ans = []
    #     ans.append((group[1]['answers'], 1))
    #     random.shuffle(all_fb_ans)
    #     iterator = iter(all_fb_ans)
    #     while (len(ans) != 4) and (len(ans) != len(all_fb_ans)):
    #         val = next(iterator)
    #         if val != group[1]['answers']:
    #             ans.append((val, 0))
    #     random.shuffle(ans)
    #     for i, item in enumerate(ans):
    #         temp[i+1] = item[0]
